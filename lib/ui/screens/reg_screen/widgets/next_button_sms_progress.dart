...
import 'package:unyapp/ui/screens/reg_screen/widgets/login_error.dart';

class NextButtonSmsProgress extends StatelessWidget {
  final void Function() successCallback;
  final void Function() errorCallback;

  const NextButtonSmsProgress({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    switch (loginState.status) {
      ...
      case XStatus.failure:
        if (loginState.errorCode == 401) {
          return LoginError(errorCallback: errorCallback);
        } else {
          return ActionError(
            errorMessage: loginState.errorMessage ?? 'Error',
      ...
      case XStatus.success:
        return ActionSuccess(
          ...
          successAction: () => RegScreenSmsCodeActions.gotoGenderInputScreen(context),
        );
    }
  }
}
