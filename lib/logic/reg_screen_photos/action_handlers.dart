...
import 'package:unyapp/logic/reg_screen_photos/delete_photo_cubit.dart';
...
abstract class RegScreenPhotoActions {
  //---
  ...
  //   //-------
  //   final addInterestCubit =
  //       BlocProvider.of<AddInterestCubit>(context, listen: false);
  //   addInterestCubit.addInterestToUser(interest.id);
  // }

  //---
  // Нажатие на иконку "закрыть" InterestChip (удаление интереса из выбранных)
  static void photoItemOnCloseIconTap(BuildContext context, Interest interest) {
    // final provider = Provider.of<RegScreenInterestsProvider>(context, listen: false);
    // provider.removeInterestFromSelected(interest);
    //-------
    final deleteInterestCubit =
        BlocProvider.of<DeletePhotoCubit>(context, listen: false);
    deleteInterestCubit.deletePhotoFromUser(interest.id);
  }
  ...
  static String categoryLimitErrorMessage(BuildContext context) {
    ...
    if (count < 20) return '';
    final categoryName =
        unyCategories.firstWhere((el) => el.id == curCategoryId).name;
